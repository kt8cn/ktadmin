...
class KtModel extends BaseModel
{
	...
	*/
	public static function ktapp($req)
	{
		...
		// $data['page'] = $page;
		// $data['size'] = $size;
		$res = Db::table('kt_base_market_app')->field('logo,code,name,app_type,install_type,author,version,c_time');
		if($where) $res->where($where);
		// $data['count'] = $res->count();
		// $data['item'] = $res->page($page,$size)->select();
		return success('获取成功',$res->select());
	}
	/**
	* 删除应用
	*/
	...
	public static function syncAppStore(){
		...
		if(isset($res['data'])){
			foreach ($res['data'] as $value) {
				...
				Db::table('kt_base_app_store')->save($data);
			}
		}
	}
