...
class Login extends BaseUser
{
    public function index()
    {
		...
		if($user['pwd'] != md5($password)) return error('帐号或密码错误');
    ...
    */
    public function updatePwd()
    {
        ...
        if($user['pwd'] != md5($password)) return error('当前密码错误');
        ...
        if($user['pwd'] == md5($new_password)) return error('新旧密码一致');
    ...
    **/
    public function fogretpwd(){
        ...
        Db::table('kt_base_user')->where('id',$user['id'])->update(['pwd'=>md5($password)]);
        Cache::delete($key);
        return success('修改成功');
    }  
}