...
<script setup>
  ...
  import { template,usetemplate } from '@/api'
  ...
  const editapi=(val)=>{
    usetemplate({code:val.code}).then(res=>{
      ElMessage({
        message: res.msg,
        type: 'success',
      })
  ...
  <template>
    ...
    <div v-else class="contentCenter">
      ...
      <div class="contentFather">
        <div  v-for="(item, index) in appList" :key="index" @click="jumpapi(item)" :class="item.isuse == 1?'contentChild bgc-c':'contentChild '">
          <div  >
            ...
            <div style="margin-left: 7px">
						<el-button type="primary" size="small" plain @click.stop="editapi(item)" :disabled='item.isuse == 1?true:false'>启用</el-button>
					</div>
          </div>
